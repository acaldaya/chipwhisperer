...
# from https://github.com/mbrown1413/des
...
def IP(input):
    ...
    input = uint64(input)
    for i in range(0, 64):
        if input & initial_permutation[i]:
            output |= 1 << (63 - i)
    return output

def Left(input):
    return uint64(input) >> uint8(32)
    
def Right(input):
    return uint64(input) & uint64(0xFFFFFFFF)

def Expand(input):
    output = 0
    for i in range(0, 48):
        if input & expansion[i]:
            output |= 1 << (47 - i)
    return output
